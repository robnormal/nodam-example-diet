...
(function() {
  var Async, DELETE, GET, M, POST, PUT, R, actions, addIngredient, addMealToPlan, apology, createFood, createMealFood, createPlan, createWeek, db, dbM, deleteIngredient, endsWith, fmap, foodUrl, fs, getLatestMeal, getMatchedWeek, jade, logError, mealUrl, mimeTypes, nodam, orm, path, planUrl, qs, queries, removeMealFromPlan, reorderPlanMeals, routes, serveCompressed, serveFile, sql, toInt, updateFood, updateIngredient, updateMealFood, updateWeek, util, web, weekUrl, _;
  ...
  actions = {
    root: function(match) {
      return orm.Nutrient.get({
        name: 'carbohydrates'
      }).pipeMaybe(web.error403('WTF?! Recreate "carbohydrates" nutrient.'), function(carbs_nut) {
        return db.allFoods.pipeMapM(function(food) {
          return orm.foodNutrient(food, carbs_nut).pipe(function(carbs) {
            return food.set('carbs', carbs);
          });
        }).pipe(function(foods) {
    ...
    mealNutrients: function(match) {
      var meal_name;
      meal_name = web.uriToWord(match[1]);
      return orm.Meal.get({
        name: meal_name
      }).pipeMaybe(web.error403('could not find meal ' + meal_name), function(meal) {
        return orm.allNutrientsIn(orm.mealIngredients(meal)).pipe(function(amts) {
          return web.showView('mealNutrients', {
            meal: meal,
            amts: amts
          });
        });
