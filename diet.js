...
(function() {
  var DELETE, GET, M, POST, PUT, R, actions, addMealToPlan, allMeals, createFood, createIngredient, createMealFood, createPlan, dbAll, dbEach, dbFunction, dbGet, dbM, dbQueryFunction, dbRun, deleteFood, deleteIngredient, deleteMealFood, display, error403, error404, fillMealFoods, fmap, foodUrl, fs, getIngredients, getJade, getPost, getView, helper, jade, logError, matchUrl, mealUrl, model, nodam, orm, planUrl, qs, queries, redirect, routeRequest, routes, runQuery, setMealCals, showMonadErr, showView, sql, success, updateFood, updateMealFood, uriToWord, wordToUri, _,
    __slice = [].slice;
  ...
  actions = {
    ...
    manageMeals: function(match) {
      return nodam.combine([dbM, getPost]).pipeArray(function(db_obj, post) {
        if (post['delete']) {
          ...
          return runQuery(queries.meals_insert, {
            name: post.name
          }).then(dbGet(queries.meals + orm.condition({
    ...
    meal: function(match) {
      ...
      return dbGet(queries.meals + orm.condition({
        ...
        if (!meal) {
          ...
          return fillMealFoods(meal).pipe(function(mealFilled) {
    ...
    mealFoods: function(match) {
      ...
      return nodam.combine([dbM, getPost]).pipeArray(function(db_obj, post) {
        return dbGet(queries.meals + orm.condition({
          ...
          e_m = post.meal_name ? M.right(dbRun("UPDATE meals SET name='" + post.meal_name + "' WHERE id=" + meal_id)) : post['delete'] ? M.right(deleteMealFood(meal_id, post['delete'])) : post.create ? M.right(createMealFood(meal, post)) : post.update ? M.right(updateMealFood(meal, post)) : M.left('Invalid form submission.');
    ...
    planMeals: function(match) {
      ...
      return dbGet(queries.plans + orm.condition({
        ...
        return dbAll(queries.plan_meals_with_meals + orm.condition({
          ...
          return _.fmap(model.hydrateCommon, rows);
        }).pipe(function(meals) {
          if (meals && meals.length) {
            console.log('meals:', meals);
            return nodam.sequence(_.map(meals, fillMealFoods));
          } else {
            return nodam.result([]);
          }
        }).pipe(function(mealsFilled) {
    ...
    managePlan: function(match) {
      return nodam.combine([dbM, getPost]).pipeArray(function(db_obj, post) {
        var m, plan_name;
        console.log(post);
        m = post.create ? createPlan(post) : (plan_name = match[1] && uriToWord(match[1]), dbGet(queries.plans + orm.condition({
          name: plan_name
        })).pipe(function(plan) {
          if (!plan) {
            ...
            return addMealToPlan(plan, post);
          } else {
  ...
  createPlan = function(post) {
    if (post.name) {
      return M.right(runQuery(queries.plans_insert, {
        name: post.name
  ...
  addMealToPlan = function(plan, post) {
    if (!post.meal_name) {
      return nodam.result(M.left('Invalid form submission.'));
    }
    return model.mealByName(post.meal_name).pipe(function(meal) {
      if (!meal) {
        return nodam.result(M.left('No meal exists by that name'));
      }
      return runQuery(queries.plan_meals_insert, {
        plan_id: plan.id,
        meal_id: meal.id
