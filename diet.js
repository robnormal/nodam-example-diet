...
(function() {
  var Async, DELETE, GET, M, POST, PUT, R, actions, addIngredient, addMealToPlan, apology, createFood, createMealFood, createPlan, createWeek, db, dbM, deleteIngredient, fmap, foodUrl, fs, getLatestMeal, getMatchedWeek, jade, logError, mealUrl, mimeTypes, nodam, orm, path, planUrl, qs, queries, removeMealFromPlan, reorderPlanMeals, routes, serveFile, sql, toInt, updateFood, updateIngredient, updateMealFood, updateWeek, util, web, weekUrl, _;
  ...
  logError = function(err) {
    ...
    return fs.appendFile('errors.log', log);
  };
  ...
  actions = {
    root: function(match) {
      return orm.foodsWithIngredients({}, {
        order_by: 'foods.name'
      }).pipe(function(foods) {
    ...
    manageMeals: function(match) {
      return nodam.combineStrict([dbM, web.getPost]).pipeArray(function(db_obj, post) {
        if (post['delete']) {
          ...
          return db.createMeal(post.name).then(getLatestMeal).pipeMaybe(web.error403(apology), function(meal) {
    ...
    mealFoods: function(match) {
      ...
      return nodam.combineStrict([dbM, web.getPost]).pipeArray(function(db_obj, post) {
        ...
        return changes.then(web.redirect(match[0])).rescueOnly(Error, function(err) {
          return logError(err).then(nodam.failure('There was a problem with your request'));
        }).rescue(web.error403);
      });
    },
    ...
    plans: function(match) {
      return orm.Plan.find().pipe(function(plans) {
    ...
    planMeals: function(match) {
      ...
      return orm.Plan.get({
        ...
        return orm.planWithFoods(plan).pipe(function(plan) {
